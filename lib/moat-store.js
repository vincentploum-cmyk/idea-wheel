 ...
 */
export async function getValidationEligibility(validationId, { minScore = 60, requiredVersion = null } = {}) {
  ...
  if (supabase) {
    try {
      ...
      console.error('[moat-store/getValidationEligibility]', err.message);
    }
  }
  // Local JSONL fallback (dev / migration-not-applied).
  const rows = await readJsonl(FILES.validations, 500);
  const match = [...rows].reverse().find((row) => String(row.id) === id);
  if (match) return evaluate(match.eval, match.scout);
