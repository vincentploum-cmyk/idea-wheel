...
export async function getValidationEligibility(validationId, { minScore = 60, requiredVersion = null, userId = null } = {}) {
  ...
  // Ownership check — fails CLOSED, including for legacy null-owner rows.
  // A validation owned by someone else (or by nobody we can attribute) must look
  // like it doesn't exist so existence can't be fingerprinted. Legacy rows from
  // before user_id was added are unreachable by design: they cannot be safely
  // attributed to any user, and letting anyone use them would weaken the gate.
  // The affected user simply re-runs their validation. A security-event log
  // captures each hit so we can watch this decay to zero after migration.
  const ownsRow = (row) => {
    if (!userId) return false; // ownership required; no caller may opt out
    const rowUser = row?.user_id ?? row?.userId ?? null;
    if (rowUser === null || rowUser === undefined) {
      console.warn('[security] legacy_validation_without_owner', JSON.stringify({
        event: 'legacy_validation_without_owner',
        validationId: id,
  ...
  const evaluate = (evalObj, scout) => {
    // Guard BEFORE Number() coercion — Number(null) === 0 (finite), so without
    // this a null score falls through to "below_threshold" instead of "unscored".
    const raw = evalObj?.scores?.overall;
    if (raw === null || raw === undefined) return fail('unscored', { found: true });
