...
export const CREDIT_PACKS = [
  ...
  { id: 'pro',     label: 'Pro',     type: 'idea', credits: 0,  ideaCredits: 1, price_cents: 999,  price_display: '$9.99',  tagline: 'Skip the blank page and unlock 1 researched starting point' },
  { id: 'power',   label: 'Power',   type: 'idea', credits: 0,  ideaCredits: 2, price_cents: 1999, price_display: '$19.99', tagline: 'Unlock 2 researched starting points and choose the stronger one' },
];
...
// In-process mutex to narrow the race window for concurrent deduction requests
// from the same user. Full fix requires a Supabase RPC with serializable transaction.
const _deductLocks = new Map();

// Deduct credits — checks balance then inserts debit row.
// Race condition note: this is guarded by an in-process mutex per user. A proper
// fix requires a DB-level RPC (SELECT SUM(change) FOR UPDATE + INSERT in one txn).
export async function deductCredits(userId, amount, reason, metadata = {}) {
  // Acquire in-process lock for this user
  while (_deductLocks.get(userId)) {
    await new Promise((r) => setTimeout(r, 20));
  }
  _deductLocks.set(userId, true);
  try {
    const db = getAdmin();
