...
// A Postgres unique-violation means a concurrent request already recorded this
// row (the idempotency indexes in supabase/credit-integrity.sql). Harmless until
// those indexes exist, at which point it's how races are made safe.
function isUniqueViolation(error) {
  return error?.code === '23505' || /duplicate key value|unique constraint/i.test(error?.message || '');
}

export const CREDIT_COSTS = { spin: 1, blueprint: 2, ideaUnlock: 1 };
...
// Detects both the current reason (`catalog_unlock_<slug>`, paid from the regular
// credit balance) and the legacy `idea_unlock_<slug>` so earlier unlocks still count.
export async function hasUnlockedIdea(userId, slug) {
  ...
  const { data } = await db
    ...
    .select('id')
    .eq('user_id', userId)
    .in('reason', [`catalog_unlock_${slug}`, `idea_unlock_${slug}`])
    .limit(1);
  return !!data?.length;
}

// Site-wide ideas-library unlock — the one-time charge recorded by
// /api/ideas-unlock with reason 'ideas_unlock'.
...
// Per-idea catalog blueprint unlock — the charge recorded by
// /api/catalog-blueprint-unlock with reason `idea_blueprint_<slug>`.
...
export async function deductCredits(userId, amount, reason, metadata = {}) {
  ...
  if (error) {
    // Unique-violation from the unlock idempotency index = a concurrent request
    // already recorded this exact unlock. Not a failure — signal a safe duplicate
    // so the caller can treat it as "already unlocked" instead of double-charging.
    if (isUniqueViolation(error)) return { ok: false, duplicate: true, reason: 'already_recorded' };
    throw new Error(`deductCredits RPC failed: ${error.message}`);
  }
  if (!data.ok) {
...
export async function addCredits(userId, amount, reason, metadata = {}) {
  ...
  if (error) {
    // Unique-violation = a concurrent grant already recorded this (purchase or
    // review_bonus idempotency index). Safe duplicate, not a failure.
    if (isUniqueViolation(error)) {
      return { ok: true, duplicate: true, newBalance: await getBalance(userId) };
    }
    throw new Error(`addCredits failed: ${error.message}`);
  }
