...
/** Every value of a flat record becomes text; non-records pass through. */
...
export function normalizeDesign(design) {
  ...
  if (Array.isArray(out.evidenceVerified)) {
    ...
    out.evidenceVerified = out.evidenceVerified.filter(isRecord).map((entry) => ({
      ...entry,
