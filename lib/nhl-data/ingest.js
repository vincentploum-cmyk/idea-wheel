...
import { snapshotPositions, applyFrozenPositions, positionsPath } from './positions';
...
export async function ingestGames(date, { force = false } = {}) {
  ...
  // Skaters keep the position they were frozen at before the game.
  if (fresh.length) applyFrozenPositions(fresh, await readJson(positionsPath(date)));
...
export async function ingestLineups(date) {
  ...
  const lineups = { date, fetchedAt: new Date().toISOString(), games };
  ...
  return { date, scheduled: schedule.length, withLineups: found, positions };
}
...
export async function dailyRefresh(today) {
  ...
  const summary = { ranAt: new Date().toISOString(), today, yesterday: games, todayGames: gamesToday, lineups, preseason, rosters, league, media };
