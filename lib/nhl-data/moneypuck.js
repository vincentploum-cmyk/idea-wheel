...
async function fetchCsv(url) {
  const res = await fetch(url, { headers: UA, cache: 'no-store', signal: AbortSignal.timeout(20000) });
  if (!res.ok) { const err = new Error(`MoneyPuck answered ${res.status} for ${url}`); err.status = res.status; throw err; }
...
export async function refreshMoneyPuck({ year = mpYear(), games = true } = {}) {
  ...
  let rows;
  try {
    rows = await fetchCsv(`${BASE}/seasonSummary/${year}/regular/teams.csv`);
  } catch (err) {
    // Before opening night the new season's file doesn't exist yet: use last season.
    if (err.status !== 404) throw err;
    year -= 1;
    rows = await fetchCsv(`${BASE}/seasonSummary/${year}/regular/teams.csv`);
  }
  if (!rows.length || !('situation' in rows[0])) throw new Error(`MoneyPuck teams.csv for ${year} has an unexpected layout (columns: ${Object.keys(rows[0] || {}).slice(0, 8).join(', ')})`);
  const bySituation = {};
  for (const r of rows) {
    const team = toAbbr(r.team);
...
/** The newest stored snapshot: this season's, or last season's until the new one exists. */
export async function loadMoneyPuck(year = mpYear()) {
  for (const y of [year, year - 1]) {
    const [teams, games] = await Promise.all([readJson(MP_TEAMS(y)), readJson(MP_GAMES(y))]);
    if (teams) return { teams, games };
  }
  return { teams: null, games: null };
}
