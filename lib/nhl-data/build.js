...
export async function buildBoxScores(date) {
  ...
  const line = (s) => [s.name, s.g, s.a, s.pm, s.sog, Math.max(0, s.iff - s.isf), s.blk, s.pim ? 1 : 0, s.pim, s.hits, s.tk, s.gv, s.shifts, s.toi, '', ''];
...
export async function buildHistorical(asOf, { days = 365 } = {}) {
  ...
  const rows = (await loadRows(asOf)).filter((r) => r.date >= from);
  ...
  const sheets = Object.keys(groups).sort().map((key) => {
    ...
    const body = list.flatMap((r) => [
      [r.date, r.result, r.venue, r.name, 'N/A', r.toi, r.g, r.a, r.g + r.a, r.sog, r.hits, r.blk],
...
export async function buildPlayerStats(asOf, { window = 82, minGames = 5 } = {}) {
  const rows = await loadRows(asOf);
  if (!rows.length) return null;
  const byPlayer = {};
