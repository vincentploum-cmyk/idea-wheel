...
import { SCORE_POLICY, hasPotential } from './score-policy';

// Eligibility columns for a saved row, kept consistent with the DB check
// constraints in supabase/saved-ideas-eligibility.sql: "eligible" is only ever
// set alongside a >=60 score + version + scored_at. A missing score is 'pending'.
function eligibilityFields(score) {
  const n = Number(score);
  if (!Number.isFinite(n)) {
    return { score_version: null, scored_at: null, eligibility_status: 'pending' };
  }
  return {
    score_version: SCORE_POLICY.version,
    scored_at: new Date().toISOString(),
    eligibility_status: hasPotential(n) ? 'eligible' : 'rejected',
  };
}
...
export async function saveValidatedIdea({ userId, validationId, idea = {}, comp = {} }) {
  const db = getAdmin();
  if (!db) return { error: 'no_service_role_key' };
  ...
  try {
    ...
    if (error) return { error: error.message || 'db_error', code: error.code };
    return { id: data.id };
  } catch (e) {
