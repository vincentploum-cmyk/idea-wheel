...
export function computeCostModel(infra, { monthlyPrice = null } = {}) {
  ...
  // Enforce the floors models routinely forget.
  if (!priced.some((p) => HOSTING_RE.test(p.service))) {
    priced.push({ service: 'Hosting (Render Pro baseline)', quantity: 1, unit: 'workspace', unitCost: HOSTING_BASELINE, monthlyCost: HOSTING_BASELINE });
    notes.push('Added a hosting baseline (Render Pro) — the model omitted a hosting line.');
  }
  if (!priced.some((p) => DB_RE.test(p.service))) {
    priced.push({ service: 'Database (managed Postgres baseline)', quantity: 1, unit: 'instance', unitCost: DB_BASELINE, monthlyCost: DB_BASELINE });
    notes.push('Added a database baseline — the model omitted a database line.');
  }
