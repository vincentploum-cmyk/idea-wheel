 ...
 */
export async function recordCandidate({ mode, action, workflow, industry, score, safetyLevel = 'standard', title, summary, gap, comp = null, agentDesc = null }) {
  const db = getAdmin();
 ...
 */
export async function getCandidateEligibility(mode, workflow, industry, { minScore = 60, requiredVersion = null } = {}) {
  ...
  try {
    const { data } = await db
      .from('idea_candidates')
      .select('viability_score, score_version, safety_level, eligibility_status')
      .eq('combo_key', comboKey)
      .maybeSingle();
