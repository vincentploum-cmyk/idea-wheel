...
function buildWorkflowIndustryWeights(selectedWorkflows, selectedIndustries, scorecard, seedPrior = null) {
  const result = {};
...
export function buildAdaptiveGeneratorConfig(trainingData = {}, opportunityBank = []) {
  ...
  for (const [modeKey, defaults] of Object.entries(DEFAULT_MODE_CONFIGS)) {
    ...
    const seededBanks = [
      mergeSeededBank(defaults.banks[0], seedPrior.actions),
      mergeSeededBank(defaults.banks[1], seedPrior.workflows),
      mergeSeededBank(defaults.banks[2], seedPrior.industries),
    ];
    ...
    const workflowIndustryWeights = buildWorkflowIndustryWeights(workflowBank.values, industryBank.values, card, seedPrior);
...
export function buildGeneratorIdea(modeConfig, action, workflow, industry, seed = null) {
  ...
  const reelDescription = modeConfig.name === 'B2B'
    ? `${action} ${workflow} for ${industry}`
    : `${action} ${workflow} ${modeConfig.connector} ${industry}`;
  return {
    action,
    workflow,
    industry,
