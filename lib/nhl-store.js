...
export async function readJson(path) {
  return getJson(await bucket(), path);
}

export async function writeJson(path, value) {
  return putJson(await bucket(), path, value);
}

export async function writeBlob(path, blob, contentType) {
  const store = await bucket();
  const { error } = await store.upload(path, blob, { upsert: true, contentType: contentType || 'application/octet-stream' });
  if (error) throw error;
}
