...
const jsonCache = new Map();    // path → { value, at }
...
export async function getSessionUser() {
  ...
  const key = all.filter((c) => /^sb-.*-auth-token/.test(c.name)).map((c) => `${c.name}=${c.value}`).join(';');
  if (!key) return null;
...
export async function readJson(path) {
  ...
  if (fresh(hit, JSON_TTL_MS)) return hit.value;
  const value = await getJson(await bucket(), path);
  // Misses are cached too (a missing file is a common, cheap answer), but briefly.
  jsonCache.set(path, { value, at: value === null ? Date.now() - JSON_TTL_MS + 30 * 1000 : Date.now() });
  return value;
}

export async function writeJson(path, value) {
  invalidate(path);
  await putJson(await bucket(), path, value);
  jsonCache.set(path, { value, at: Date.now() });
}
