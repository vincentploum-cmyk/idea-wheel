...
const FAQS = [
  {
    q: "What is Idea Generator?",
    a: "Idea Generator is an AI-powered startup ideation tool. It generates business ideas by combining actions, workflows, and target industries, then validates each idea against real market data — competitor analysis, market sizing, demand signals — before building a full product blueprint.",
  },
  {
  ...
  },
  {
    ...
    a: "Unlikely. There are 5,103 unique combinations across B2B and consumer modes — and the AI validation layer adds another dimension of specificity. Even if two users land on the same base combination, the competitor landscape, gap analysis, and blueprint output are generated fresh each time based on live market conditions.",
  },
  {
  ...
  },
  {
    ...
    a: "Yes — that's the point. The infrastructure section gives you exact services to sign up for, the build order, and realistic cost estimates. The prototype gives you a visual starting point. Most founders use the blueprint to validate with early customers before writing production code.",
  },
  {
    q: "Do credits expire?",
    a: "No. Credits are stored to your account and never expire. Buy when you need them, use them at your own pace.",
  },
  {
...
export default function FaqPage() {
  return (
    <main style={s.page}>
      ...
      <div style={s.wrap}>
        <div style={s.topbar}>
          <Link href="/" style={s.back}>← Idea Generator</Link>
          <Link href="/pricing" style={s.back}>Pricing</Link>
        </div>
        <h1 style={s.h1}>Frequently asked questions</h1>
        <p style={s.sub}>Everything you need to know about Idea Generator.</p>
        <div style={s.list}>
          {FAQS.map((f, i) => <FaqItem key={i} {...f} />)}
        </div>
        <div style={s.cta}>
          <p style={s.ctaText}>Still have questions?</p>
          <a href="mailto:hello@ideagenerator.app" style={s.ctaLink}>Send us a message →</a>
        </div>
        <div style={s.disclaimer}>
          <p style={s.disclaimerText}>Idea Generator is an AI-powered research and ideation tool. All market analysis, competitor data, build scores, and recommendations are generated by AI and provided for informational purposes only. They do not constitute professional business, legal, or financial advice.</p>
        </div>
      </div>
    </main>
  );
